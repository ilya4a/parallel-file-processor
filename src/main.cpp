...
Config parse_cli(int argc, char *argv[]) {
    ...
    auto *opt_replacement =
        app.add_option("-r,--replacement", replacement, "Replacement string (if omitted, search only)");
    ...
    app.add_flag_function(
           "-v",
           [&detail](int count) { detail = count; },
           "Increase output detail level (e.g. -v, -vv or -vvv )"
    )
    ...
    if (*opt_replacement) {
        builder.set_replacement(std::move(replacement));
    }
    ...
    if (detail > 0) {
        builder.set_detail_level(detail);
    }
    ...
    if (!extensions.empty()) {
        builder.set_extensions(std::move(extensions_vector));
    }
